...
kubeadm init --kubernetes-version=${version} --apiserver-cert-extra-sans k8s-master,`hostname -I | cut -d" " -f 1 | tr -d "\n"`,`curl ifconfig.me`  --pod-network-cidr ${pod_cidr} --service-cidr=${service_cidr} >> /var/log/kubeadm-init.log
# Deploy kube network
